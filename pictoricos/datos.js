...
// Chart Functions Pictorcs Begin
  ...
  function insLeyenda(state) {
    ...
    img.onload = function() {
      ...
      ctx.font = 'bold ' + chart.image.caption.fontSize + 'px Arial'
      ctx.textAlign = 'right'
      ctx.fillStyle = 'red'
      ctx.textBaseline = 'middle'
      let textVal = " = " + chart.image.caption.value
      ctx.fillText(textVal,0,imageH/2);
      let textWidth = ctx.measureText(textVal).width
      console.log(textWidth)

      //ctx.translate(-imageW,capH/2 - imageH/2)
      ctx.rect(0, 0, -2*capW,capH)
      ctx.stroke()
      ctx.drawImage(img, -imageW - textWidth, 0,imageW,imageH)
      //ctx.translate(-10,-10)
    }
